...
## Running the project
First, the **Database Tutorial.ipynb** file should be run, and then the **CSV Tutorial.ipynb** file. After the two CSV files are generated (they are provided here), **Cross validating the premodel.ipynb** file should be run. A note: logistic regression in the *premodel.py* file was added by me (it was not in the original source code). **Premodel_issues.ipynb** file shows some of the issues with premodel training.
